...
export function NotesPage() {
  return <WorkspaceScaffold>{({ snapshot }) => <NotesLibrary branchNodeIds={new Map(snapshot?.branches.map((branch) => [branch.id, branch.nodeId]))} branchTitles={new Map(snapshot?.branches.map((branch) => [branch.id, branch.title]))} />}</WorkspaceScaffold>;
}

function NotesLibrary({ branchTitles, branchNodeIds }: { branchTitles: Map<string, string>; branchNodeIds: Map<string, string> }) {
