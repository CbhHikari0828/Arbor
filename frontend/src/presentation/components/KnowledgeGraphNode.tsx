...
export function KnowledgeGraphNode({ data }: NodeProps<KnowledgeGraphNodeModel>) {
  ...
  return (
    <div
      className={[
        "relative w-[252px] rounded-[18px] bg-white px-5 py-4 text-sm text-[#303934] transition-colors duration-300",
        "dark:bg-[#111820]/95 dark:text-[#e7eee9]",
        data.isActive
          ? "border-2 border-[#174f42] shadow-[0_22px_55px_rgba(35,58,47,0.16)] dark:border-[#65bd7d] dark:shadow-[0_22px_60px_rgba(0,0,0,0.4)]"
          : "border border-[#e5ded4] shadow-[0_14px_36px_rgba(57,45,31,0.09)] dark:border-[#303a44] dark:shadow-[0_18px_46px_rgba(0,0,0,0.34)]",
      ].join(" ")}
    >
      ...
      <button
        ...
        className={[
          ...
          data.isFavorite
            ? "bg-[#fff5dc] text-[#d38a19] hover:bg-[#ffedbd] dark:bg-[#3a2d17] dark:text-[#f1bd54] dark:hover:bg-[#4c3a1b]"
            : "text-[#8a958e] hover:bg-[#f1f3ef] hover:text-[#b87919] dark:text-[#96a39c] dark:hover:bg-[#1d2630] dark:hover:text-[#e7b85d]",
        ].join(" ")}
        onClick={(event) => {
      ...
      <p className="min-h-[48px] text-[14px] leading-6 text-[#5e6862] dark:text-[#aab5af]">
        {data.description}
      </p>

      <div className="mt-5 flex items-center justify-between text-[13px] font-medium text-[#59645f] dark:text-[#b9c4be]">
        <span className="inline-flex items-center gap-2">
          {data.isRoot ? <FileText size={15} /> : <GitBranch size={15} className="text-[#d99a2a]" />}
      ...
      {data.isActive ? (
        <div className="absolute -bottom-3 -right-3 flex items-center gap-2">
          <button
            aria-label="删除节点"
            className="nodrag nopan grid size-10 place-items-center rounded-full border-2 border-white bg-[#fff7f4] text-[#bf4d37] shadow-[0_12px_24px_rgba(135,50,35,0.16)] transition hover:bg-[#fbe5de] dark:border-[#111820] dark:bg-[#3a2020] dark:text-[#ff9984] dark:shadow-[0_14px_28px_rgba(0,0,0,0.36)] dark:hover:bg-[#4b2929]"
            onClick={(event) => {
              event.stopPropagation();
          ...
          <button
            aria-label="添加子节点"
            className="nodrag nopan grid size-10 place-items-center rounded-full border-2 border-white bg-[#174f42] text-white shadow-[0_12px_24px_rgba(23,79,66,0.28)] transition hover:bg-[#226853] dark:border-[#111820] dark:bg-[#3f8457] dark:shadow-[0_14px_28px_rgba(0,0,0,0.36)] dark:hover:bg-[#4c9965]"
            onClick={(event) => {
              event.stopPropagation();
...
function getStatusPresentation(status: KnowledgeNodeStatus) {
  if (status === "summarized") {
    return {
      iconClass: "bg-[#eef4ee] text-[#235f4c] dark:bg-[#1d3326] dark:text-[#8bd39c]",
    };
  }

  if (status === "exploring") {
    return {
      iconClass: "bg-[#eff5ef] text-[#245d4f] dark:bg-[#1d3326] dark:text-[#9bd1a4]",
    };
  }

  return {
    iconClass: "bg-[#fff3e5] text-[#c9792f] dark:bg-[#342816] dark:text-[#f0b84f]",
  };
}
